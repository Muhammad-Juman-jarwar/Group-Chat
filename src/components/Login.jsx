...
const Login = () => {
    const {userCredentials, setUserCredentials, getUserLogin, message, loading, setMessage, user} = useDataBase()
    ...
    return (
        <div className="sm:h-[40rem] sm:w-functional-screens sm:rounded-lg w-screen h-full sm:shadow-gray-900 sm:shadow-xl flex flex-col justify-evenly">
            ...
            <form className="w-full flex flex-col gap-10 px-8" onSubmit={getLogIn}>
                ...
                <div className="relative">
                    <button type="submit" disabled={loading} className={`bg-[#4bb4f8] w-full text-lg p-2 rounded transition-all tranform ${loading? null: 'active:scale-95'} ${loading? 'opacity-80': 'opacity-100'}`}>{loading ? <img src={loader} alt="loading..." className="w-9 mx-auto"/>: 'Login'}</button>
                    <p className={`text-sm pt-2 ${message.flag === 'red'? 'text-pink-700': 'text-green-700'}`}>{message.message}</p>
                </div>
            </form>
            <p className="text-sm text-center">create an account? <Link to={'/signup'} onClick={() => {setMessage({flag: '', message: ''})}} className="text-[#4bb4f8] font-semibold cursor-pointer">Signin</Link></p>
        </div>
    )
}
